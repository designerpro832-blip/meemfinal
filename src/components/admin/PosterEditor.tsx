...
const PosterEditor: React.FC<PosterEditorProps> = ({ template, onClose, onSave }) => {
  const [fullTemplate, setFullTemplate] = useState<PosterTemplate | null>(null);
  ...
  useEffect(() => {
    const loadTemplate = async () => {
      ...
      if (template.styles) {
        ...
        const { data, error } = await supabase.from('poster_templates').select('*').eq('id', template.id).single();
        if (error) {
  ...
  useEffect(() => {
    if (fullTemplate) {
      const layout = fullTemplate.layout_name;
      const defaults = defaultPosterStyles[layout] || {};
      const dbStyles = fullTemplate.styles || {};
      ...
      setBackgroundPreview(fullTemplate.background_image_url);
      setDimensions({ width: fullTemplate.width || 500, height: fullTemplate.height || 500 });
    }
  }, [fullTemplate]);
  ...
  const handleSave = async () => {
    ...
    try {
        if (newBackgroundFile) {
            ...
            const uniqueId = fullTemplate.component_name || `${fullTemplate.name.replace(/\s+/g, '-')}-${Date.now()}`;
            newFilePath = `public/${uniqueId}.${fileExtension}`;

            const { error: uploadError } = await supabase.storage
                .from('poster_backgrounds')
                .upload(newFilePath, newBackgroundFile, { upsert: true });
            ...
            if (fullTemplate.background_image_url) {
                try {
                    const urlParts = fullTemplate.background_image_url.split('/poster_backgrounds/');
                    if (urlParts.length > 1) {
                        oldImagePath = decodeURIComponent(urlParts[1]);
                        if (oldImagePath === newFilePath) oldImagePath = null;
                    }
                } catch (e) {
  ...
  const handleLayoutSelection = (newLayout: string) => {
    ...
    const existingStyles = fullTemplate.styles || {};
  ...
  const styleGroups = {
    ...
    'Winner 1': ['winner1Name', 'winner1Team'],
    'Winner 2': ['winner2Name', 'winner2Team'],
    'Winner 3': ['winner3Name', 'winner3Team'],
      ...
      <div className="bg-gray-100 rounded-lg shadow-2xl w-full max-w-7xl h-[95vh] flex flex-col">
        <header className="flex items-center justify-between p-4 border-b bg-white rounded-t-lg">
          ...
          <div className="flex items-center gap-3">
            <button onClick={handleSave} disabled={isSaving || isLoading || !currentLayout} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 disabled:opacity-50">
                ...
                <div className="border border-gray-200 rounded-lg">
                  ...
                  {openSections['Poster Setup'] && (
                    <div className="p-4 border-t border-gray-200 space-y-4">
                      <div>
                        <label className="text-sm font-semibold text-gray-600 flex items-center gap-2 mb-2"><Layout size={16} /> Layout</label>
                        <select
                          value={currentLayout}
                          onChange={(e) => handleLayoutSelection(e.target.value)}
